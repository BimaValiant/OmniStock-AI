        ...
        <aside class="w-64 bg-[#0d1322] border-r border-gray-800/80 flex flex-col p-5 overflow-y-auto select-none">
            <div class="flex-shrink-0">
                ...
                <nav class="space-y-1.5">
                    ...
                    <a href="{{ url('/settings') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 {{ request()->is('settings') ? 'bg-slate-700/60 text-white font-semibold shadow-sm' : 'text-gray-400 hover:bg-gray-800/50 hover:text-gray-200' }}">
                        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex-1 flex flex-col bg-slateBg overflow-hidden">
            ...
            <header class="h-14 border-b border-slateBorder bg-[#0b1220] px-6 flex items-center justify-between flex-shrink-0">
                <div class="relative flex-1 max-w-[360px]">
                    <input type="text" placeholder="Search settings..." class="w-full bg-[#111622] border border-slateBorder/80 rounded-lg py-2 pl-9 pr-3 text-xs text-slate-200 focus:outline-none focus:border-slate-500 placeholder-slate-500">
            ...
            <main class="flex-1 overflow-y-auto p-6 md:p-8">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-[220px_minmax(0,1fr)] gap-6">
                    <div class="bg-slateCard border border-slateBorder rounded-xl p-3 h-fit">
                        <nav class="space-y-2 text-sm">
                            <button class="w-full text-left px-3 py-2 rounded-lg bg-slate-200 text-slate-900 font-semibold">General</button>
                            <button class="w-full text-left px-3 py-2 rounded-lg text-slate-300 hover:bg-slate-800/40 transition">Profile</button>
                            <button class="w-full text-left px-3 py-2 rounded-lg text-slate-300 hover:bg-slate-800/40 transition">Notifications</button>
                            <button class="w-full text-left px-3 py-2 rounded-lg text-slate-300 hover:bg-slate-800/40 transition">Team</button>
                            <button class="w-full text-left px-3 py-2 rounded-lg text-slate-300 hover:bg-slate-800/40 transition">Billing</button>
                        </nav>
                    </div>

                    <div class="bg-slateCard border border-slateBorder rounded-xl p-6">
                        <h3 class="font-headline font-bold text-xl text-white mb-6">Organization Profile</h3>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                            <div>
                                <label class="block text-xs text-slate-400 mb-2">Organization Name</label>
                                <input type="text" value="Acme Global Logistics" class="w-full bg-[#0B0F19] border border-slateBorder rounded-lg px-3 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:border-slate-500">
                            </div>
                            <div>
                                <label class="block text-xs text-slate-400 mb-2">Primary Currency</label>
                                <select class="w-full bg-[#0B0F19] border border-slateBorder rounded-lg px-3 py-2.5 text-white focus:outline-none focus:border-slate-500">
                                    <option>IDR</option>
                                    <option>USD</option>
                                    <option>EUR</option>
                                </select>
                        ...
                        <div class="bg-[#0B0F19] border border-slateBorder rounded-xl p-5 mb-6">
                            <div class="flex items-center justify-between mb-4">
                                <h4 class="font-headline font-bold text-lg text-white">AI Forecasting Engine</h4>
                                <span class="text-[10px] bg-slate-800/80 text-slate-300 px-2 py-1 rounded-full border border-slateBorder">Balanced</span>
                            </div>
                            <div class="mb-5">
                                <div class="flex items-center justify-between text-xs text-slate-300 mb-2">
                                    <span>Forecasting Intensity</span>
                                    <span class="text-white font-medium">Balanced</span>
                                </div>
                                <div class="h-2 rounded-full bg-[#1b2430] overflow-hidden">
                                    <div class="h-full w-2/3 bg-white rounded-full"></div>
                                </div>
                            </div>
                            <div class="rounded-lg border border-slateBorder bg-[#111827] px-4 py-3 text-xs text-slate-300 leading-relaxed">
                                The AI currently attempts to balance holding costs with a 95% service level target based on your 12-month historical sales data.
                            </div>
                        </div>

                        <div class="flex justify-end gap-3">
                            <button class="px-5 py-2.5 rounded-lg border border-slateBorder bg-[#0B0F19] text-slate-200 text-sm hover:bg-slate-900 transition">Cancel</button>
                            <button class="px-5 py-2.5 rounded-lg bg-slate-200 text-slate-900 font-semibold text-sm hover:bg-white transition">Save Changes</button>
                        </div>
                    </div>
                </div>
            </main>
